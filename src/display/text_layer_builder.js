...
/** @typedef {import("./text_layer_images.js").TextLayerImages} TextLayerImages */
...
function getDefaultTextContentParams() {
  const includeTextEditSourceRefs =
    globalThis.PDFViewerApplication?.nativeTextEditService?.enabled === true ||
    globalThis.PDFJSNativeTextEditDebug?.enabled === true;
  return {
    includeMarkedContent: true,
    disableNormalization: true,
    ...(includeTextEditSourceRefs ? { includeTextEditSourceRefs } : null),
  };
}
...
class TextLayerBuilder {
  ...
  async render({ viewport, images, textContentParams = null }) {
    ...
    this.#textLayer = new TextLayer({
      textContentSource: this.pdfPage.streamTextContent(
        textContentParams || getDefaultTextContentParams()
      ),
      images,
      container: this.div,
      viewport,
  ...
  getTextEditSource(textDiv) {
    return this.#textLayer?.getTextEditSource(textDiv) || null;
  }

  #dispatchFreeTextFonts() {
    if (!this.#eventBus) {
      return;
    }
    const fonts = [];
    for (const [fontName, style] of Object.entries(
      this.#textLayer.textStyles
    )) {
    ...
    this.#eventBus.dispatch("freetextfontschanged", {
      source: this,
      fonts,
    });
  }
